...
do
	...
	#gunzip -c $DDIR/chr${chr}_N3775_hrc.dose.vcf.gz > ./vcf/imputed_chr$chr.vcf 
	...
	./plink --vcf ./vcf/imputed_chr${chr}.vcf \
		  ...
		  --no-sex \
		  --no-pheno \
		  --make-bed --out ./scratch/tmp
	cut -f 2 ./scratch/tmp.bim | sort | uniq -d > ./scratch/${i}.dups
	./plink --bfile ./scratch/tmp  \
		--silent \
		--no-parents \
		--no-sex \
		--no-pheno \
		--exclude ./scratch/{i}.dups \
		--make-bed --out ./binary/chr${i}
